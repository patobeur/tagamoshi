...
const _O = {
	...
	indexedFruitsByFruitsIds: {}, //all cases by it number
	...
	worldDatas: {
		...
		fruitsSheatsArray: [],
		fruitdatas: {
			fruitw: 24, // collateral display damages if changed
			fruith: 24, // collateral display damages if changed
		},
		threeIds:0,
		threesSheatsArray: [],
		threedatas: {
			threew: 24, // collateral display damages if changed
			threeh: 24, // collateral display damages if changed
		},
		mobIds: 0,
		mobSheatsArray: [],
		mobdatas: {
	...
	rewardBonus: {
		newcelldiscovered: function (mob) {
			mob._.perso.xp += 50;
	...
	roomFunctions: {
		siJeChangeDeCase: function (mob) {
			if (mob._.s.past.RoomNum != mob._.s.actual.RoomNum) {
				// siLaCaseEstInconue
				_O.roomFunctions.checkIfCelleExiste(mob);
				_O.roomFunctions.exitCase(mob);
				_O.roomFunctions.enterCase(mob);
			}
		},
		checkIfCelleExiste: function (mob) {
			let actualRoom = _O.indexedRoomsByCaseNumber[mob._.s.actual.RoomNum];
			if (typeof actualRoom === "undefined") {
				_O.roomFunctions.createNewCell(mob._.s.actual.RoomNum);

				_O.rewardBonus.newcelldiscovered(mob);

				mob.svgtextgridNum.textContent = mob._.s.actual.RoomNum;

				if(_O.worldDatas.roomIds > _O.worldDatas.mobIds*2){
					_O.fruitFunctions.addfruit(mob._.s.actual.RoomNum);
					_O.fruitFunctions.addthree(mob._.s.actual.RoomNum);
				}
			}
		},
		isThereAnyOne: function (mob) {
			...
			let enemies = [];
			let stringEnemies = "";
			let rooomes = _O.roomFunctions.getAllThe9RoomsAroundXY(
				mob._.s.actual.x,
				mob._.s.actual.y
			);
			let habitantscount = 0;
			let roomscount = 0;
			rooomes.forEach((RoomNum) => {
				...
				if (currentcase) {
					for (const key in currentcase.habitants) {
						if (Object.hasOwnProperty.call(currentcase.habitants, key)) {
							// console.log(currentcase.habitants[key])
		...
		createNewCell: function (caseNumber) {
			...
			// newgrid.addEventListener("mouseover", function gridinfo(e) {
			// 	// console.log("mouse over");
			// });
			// create rooms in indexedRoomsByCaseNumber
	...
	worldFunctions: {
		...
		createFruitsSheatsArray: function () {
			for (const key in _O.jsons.indexedFlora.fruits) {
				if (Object.hasOwnProperty.call(_O.jsons.indexedFlora.fruits, key)) {
					_O.worldDatas.fruitsSheatsArray.push(
						_O.jsons.indexedFlora.fruits[key]
					);
				}
			}
		},
	...
	fruitFunctions: {
		fruit: function () {
			let fruit = {
				fruitDivElement: _O.frontFunctions.createDiv({
					tag: "div",
					attributes: {
						className: "fruit",
						textContent: "a",
				...
				initiate: function (caseNumber) {
					_O.fruitFunctions.setNewFruitIDS(fruit);
					_O.fruitFunctions.createNewFruitDiv(fruit);
					_O.fruitFunctions.setFruitPos(fruit, caseNumber);
					_O.indexedFruitsByFruitsIds[fruit._.perso.id] = fruit;
					_O.worldCasesDiv.append(fruit.fruitDivElement);
				},
			};
			return { ...fruit };
		},
		addfruit: function (caseNumber) {
			let fruit = _O.fruitFunctions.fruit();
			fruit.initiate(fruit, caseNumber);
		},
		createNewFruitDiv: function (fruit) {
			fruit.fruitDivElement = _O.frontFunctions.createDiv({
				tag: "div",
				attributes: {
					className: "fruit",
					textContent: fruit._.sheat.ico,
				},
			});
			fruit.fruitDivElement.title =
				fruit._.perso.id + " " + fruit._.perso.immat;
			fruit.fruitDivElement.className = "food " + fruit._.sheat.model + " fff";
		},
		setFruitPos: function (fruit, caseNumber) {
			let coords = _O.indexedRoomsByCaseNumber[caseNumber].coords;
			let next = {
				x:coords.x + fruit._.s.actual.x - (_O.worldDatas.fruitdatas.fruitw / 2),
				y:coords.y + fruit._.s.actual.y- (_O.worldDatas.fruitdatas.fruitw / 2)
			}
			fruit._.s.actual.x = next.x
			fruit._.s.actual.y = next.y

			fruit.fruitDivElement.style.left = fruit._.s.actual.x + "px";
			fruit.fruitDivElement.style.top = fruit._.s.actual.y + "px";
		},
		setNewFruitIDS: function (fruit) {
			fruit._.perso.id = _O.worldDatas.fruitIds + 0;
			fruit._.perso.type = Math.floor(
				Math.random() * _O.worldDatas.fruitsSheatsArray.length
			);
			fruit._.sheat = _O.worldDatas.fruitsSheatsArray[fruit._.perso.type];
			// ---------------------------------
			let immat = "";
			for (let i = 0; i < 5; i++) {
				immat =
			...
			fruit._.perso.immat = immat;
			_O.worldDatas.fruitIds++;
		},
	},
	threeFunctions: {
		three: function () {
			let three = {
				threeDivElement: _O.frontFunctions.createDiv({
					tag: "div",
					attributes: {
						className: "three",
						textContent: "a",
				...
				update: function () {},
				initiate: function (three) {
					_O.threeFunctions.setNewthreeIDS(three);
					_O.threeFunctions.createNewthreeDiv(three);
					_O.threeFunctions.setthreePos(three, caseNumber);
					_O.indexedthreesBythreesIds[three._.perso.id] = three;
					_O.worldCasesDiv.append(three.threeDivElement);
				},
			};
			return { ...three };
		},
	...
	start: function () {
		...
		this.worldFunctions.createFruitsSheatsArray();
	...
	jsons: {
		cssString: function () {
			return (
				...
				`align-content:flex-start;transform-origin:topleft;` +
				`}` +
				// ----------------------------------------
				`.room.up{background-color:${_O.worldDatas.mobdatas.case};border-radius:12px;` +
				`}` +
				// ----------------------------------------
				...
				`width:${_O.worldDatas.fruitdatas.fruith}px;` +
				`height:${_O.worldDatas.fruitdatas.fruitw}px;` +
				// `display:flex;` +
